 ...
 * via import.meta.glob, its frontmatter parsed by the small parser below, and
 * exposed newest-first. No Node-only dependencies, so it bundles for the browser.
 */
 ...
 *
 * @param {string} file_path - Module path from import.meta.glob.
 * @param {string} raw_text - File contents.
 * @returns {blog_post} Post.
 */
function to_blog_post(file_path, raw_text) {
...
export const all_posts = Object.entries(markdown_modules)
  ...
  .sort((post_a, post_b) => post_b.date.localeCompare(post_a.date));
